...
template <class vertex>
struct Zip_Mem : public Deletable {
public:
  vertex* V;
  long n;
  long m;
  ...
  Zip_Mem(vertex* VV, long nn, long mm, long cc , void* ai, void* _inEdges = NULL)
  : V(VV), n(nn), m(mm), CliqueNum(cc), allocatedInplace(ai), inEdges(_inEdges) { }

  void del() {
    if (allocatedInplace == NULL)
    {
      for (long i=0; i < n; i++) V[i].del();
      for (long i = n; i < n+CliqueNum; i++) free(V[i].neighbors);
    } else {
...
graph(vertex* _V, long _n, long _m, Deletable* _D,uintE _C = 0) : V(_V), n(_n), m(_m),
  D(_D), CliqueNum(_C), flags(NULL), transposed(0) {}

graph(vertex* _V, long _n, long _m, Deletable* _D, uintE* _flags, uintE _C = 0) : V(_V),
  n(_n), m(_m), D(_D), flags(_flags), CliqueNum(_C), transposed(0) {}
